...
def send_slack_alerts(results, available_clients):
    ...
    for ck in ACTIVE_CLIENTS:
        ...
        for vl_rec in data["vl_summary"]:
            ...
            if d_f2 is not None and d_f2 < 0:
                t1_list.append({
                    "VL Name": vln, 
                    "ZM Name": zm_name, 
                    f"{curr_m[:3]} MTD": curr_d.get('fods', 0),
        ...
        # Render & Upload T1
        if t1_list:
            df_t1 = pd.DataFrame(t1_list).sort_values(by=f"{curr_m[:3]} MTD", ascending=False).head(10)
            styled_t1 = df_t1.style.format(_fmt_num).set_properties(
                **{'background-color': '#FFCCCC', 'color': '#C00000'}, subset=[f"Delta F{ms1}", f"Delta F{ms2}"]
            )
            img_path_t1 = f"t1_{ck}_temp.png"
            dfi.export(styled_t1, img_path_t1, dpi=300)
        ...
        if t2_list:
            ...
            styled_t2 = df_t2.style.format(_fmt_num).set_properties(
                **{'background-color': '#FFD2D2', 'color': '#8B0000'}, subset=["Red Flags"]
            )
            img_path_t2 = f"t2_{ck}_temp.png"
            dfi.export(styled_t2, img_path_t2, dpi=300)
